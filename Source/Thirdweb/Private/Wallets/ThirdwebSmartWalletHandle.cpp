                                ...
                                const FStringDelegate& ErrorDelegate)
{
	...
	UE::Tasks::Launch(UE_SOURCE_LOCATION,
	                  [InInAppWallet, ChainID, bGasless, Factory, AccountOverride, EntryPoint, SuccessDelegate, ErrorDelegate]
	                  {
		                  if (FString Error; Thirdweb::create_smart_wallet(
			                  ...
			                  TO_RUST_STRING(EntryPoint)).AssignResult(Error))
		                  {
                                          ...
                                          const FStringDelegate& ErrorDelegate)
{
	...
	FSmartWalletHandle ThisCopy = *this;
	UE::Tasks::Launch(UE_SOURCE_LOCATION,
	                  [ThisCopy, Signer, ApprovedTargets, NativeTokenLimitPerTransactionInWei, EndTime, TenYearsFromNow, SuccessDelegate, ErrorDelegate]
	                  {
		                  // FAnsiString maintains pointer stability even when moved
		                  TArray<FString> ApprovedTargetsCopy = ApprovedTargets;
		                  TArray<FAnsiString> ApprovedTargetsAnsi;
		                  TArray<const char*> ApprovedTargetsCArray;

		                  for (const FString& Target : ApprovedTargetsCopy)
		                  {
			                  FAnsiString AnsiTarget = TCHAR_TO_ANSI(*Target);
			                  ApprovedTargetsAnsi.Add(AnsiTarget);
			                  ApprovedTargetsCArray.Add(*ApprovedTargetsAnsi.Last());
		                  }


		                  if (FString Error; Thirdweb::smart_wallet_create_session_key(ThisCopy.GetID(),
		                                                                               TO_RUST_STRING(Signer),
		                                                                               ApprovedTargetsCArray.IsEmpty()
			                                                                               ? nullptr
		                                                                               ...
		                                                                               0,
		                                                                               TO_RUST_TIMESTAMP(TenYearsFromNow)).AssignResult(Error))
		                  {
...
void FSmartWalletHandle::RevokeSessionKey(const FString& Signer, const FSimpleDelegate& SuccessDelegate, const FStringDelegate& ErrorDelegate)
{
	...
	FSmartWalletHandle ThisCopy = *this;
	UE::Tasks::Launch(UE_SOURCE_LOCATION,
	                  [ThisCopy, Signer, SuccessDelegate, ErrorDelegate]
	                  {
