...
void UThirdwebOAuthExternalBrowser::Authenticate(const FString &Link) {
  ...
  FModuleManager::Get().LoadModuleWithFailureReason(FName(TEXT("HTTPServer")),
                                                    ModuleResult);
...
#else
  FHttpRequestHandler Handler = FHttpRequestHandler::CreateUObject(
      this, &UThirdwebOAuthExternalBrowser::CallbackRequestHandler);
#endif

  RouteHandle = Router->BindRoute(FHttpPath(TEXT("/callback")),
                                  EHttpServerRequestVerbs::VERB_GET, Handler);
  ...
  TW_LOG(VeryVerbose,
         TEXT("OAuth HTTP Server started and listening on port 8789"));

  bIsSiwe = Url.ToUpper().TrimStartAndEnd().Equals(TEXT("SIWE"));
  if (bIsSiwe) {
    Url = FString::Printf(
        TEXT("http://static.thirdweb.com/auth/siwe?redirectUrl=%s"),
...
bool UThirdwebOAuthExternalBrowser::CallbackRequestHandler(
    const FHttpServerRequest &Request, const FHttpResultCallback &OnComplete) {
  AuthResult = Request.QueryParams.FindRef(TEXT("authResult"));
  Signature = Request.QueryParams.FindRef(TEXT("signature"));
  Payload = FGenericPlatformHttp::UrlDecode(
      Request.QueryParams.FindRef(TEXT("payload")));
  TW_LOG(VeryVerbose,
         TEXT("UThirdwebOAuthExternalBrowser::CallbackRequestHandler::"
              "Signature=%s|Payload=%s|AuthResult=%s"),
         *Signature, *Payload, *AuthResult)
  if (bIsSiwe ? Signature.IsEmpty() || Payload.IsEmpty()
              : AuthResult.IsEmpty()) {
    FString Error = FString::Printf(TEXT("%s query parameter is missing"),
                                    bIsSiwe ? TEXT("Signature/Payload")
                                            : TEXT("AuthResult"));
    TUniquePtr<FHttpServerResponse> Response =
        FHttpServerResponse::Create(Error, TEXT("text/plain"));
    ...
    TUniquePtr<FHttpServerResponse> Response = FHttpServerResponse::Create(
        FString::Printf(
            TEXT("<script>window.location.replace('%s')</script>"),
            *UThirdwebRuntimeSettings::GetExternalAuthRedirectUri()),
        TEXT("text/html"));
    OnComplete(MoveTemp(Response));
  }
  return true;
...
void UThirdwebOAuthExternalBrowser::HandleError(const FString &Error) {
  TW_LOG(VeryVerbose, TEXT("UThirdwebOAuthExternalBrowser::HandleError::%s"),
         *Error);
  if (OnError.IsBound()) {
